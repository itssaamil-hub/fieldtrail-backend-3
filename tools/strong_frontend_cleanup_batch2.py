...
# Remove obsolete global Location Settings block from CRM settings. Employee-specific
# location/tracking settings are already rendered by React elsewhere.
...
# Native admin greeting directly in React.
...
# Native map team pulse; React already owns salesmen, map mode and route selection.
...
# Salesman dashboard KPI styling is now declared by StatCard rather than discovered after render.
old='''function StatCard({ label, value, sub, icon: IconC, color, onClick, comparison, comparisonPeriod }) {'''
new='''function StatCard({ label, value, sub, icon: IconC, color, onClick, comparison, comparisonPeriod }) {\n  const labelText = typeof label === "string" ? label : "";\n  const salesmanKpi = ["Today", "Hot", "Conversation", "Negotiation", "Won", "Renewals"].includes(labelText);'''
if old not in app: raise SystemExit('StatCard anchor missing')
app=app.replace(old,new,1)
# Update the non-dashboard className only; leave dashboard variant untouched.
