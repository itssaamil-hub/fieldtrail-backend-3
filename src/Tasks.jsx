...
export function TasksModal({lead,onClose,startCreate=false,focusId,embedded=false,active=true}) {
 ...
 useEffect(()=>{
  ...
  api.tasks(params).then(r=>{if(!cancelled){setTasks(r.tasks);setMore(r.hasMore)}}).catch(e=>{if(!cancelled)setError(e.message)}).finally(()=>{if(!cancelled)setLoading(false)});
 ...
 useEffect(()=>{
  if(!creating||lead||(!assigned&&admin))return;let cancelled=false;
 ...
 },[creating,lead,assigned,admin,leadSearch]);
 useEffect(()=>{
  if(!selectedId||!active)return;let cancelled=false;setDetailLoading(true);setDetailError('');
  api.taskDetail(selectedId).then(r=>{if(!cancelled){setDetail(r.task);setEvents(r.events||[]);setEditPriority(r.task.priority||'medium');setEditRepeat(r.task.recurrence||'none')}}).catch(e=>{if(!cancelled){setDetail(null);setDetailError(e.message)}}).finally(()=>{if(!cancelled)setDetailLoading(false)});
  return()=>{cancelled=true};
 ...
 const create=async e=>{e.preventDefault();const ok=await mutate(()=>api.createTask({title,notes,dueAt:dueISO(due),priority,recurrence,...(admin?{assignedTo:assigned}:{}),...(linkedLead?{leadId:linkedLead}:{})}),'Task created.');if(ok){setCreating(false);setTitle('');setNotes('');setDue('');setLinkedLead(lead?.id||'');setLeadSearch('');setFilter('all');setPage(0)}};
 ...
 const content=<div className={embedded?'ft-tasks-embedded':'ft-tasks-backdrop'} onClick={e=>{e.stopPropagation();if(!embedded&&!busy)onClose?.()}}>
  <section className="ft-tasks-dialog ft-tasks-workspace" ref={dialog} role={embedded?undefined:'dialog'} aria-modal={embedded?undefined:true} aria-labelledby={titleId} onClick={e=>e.stopPropagation()}>
   ...
   {creating&&<form className="ft-task-form" onSubmit={create}>
    ...
    <label>Priority<select value={priority} onChange={e=>setPriority(e.target.value)}>{['high','medium','low'].map(v=><option key={v} value={v}>{v[0].toUpperCase()+v.slice(1)}</option>)}</select></label>
    <label>Repeat<select value={recurrence} onChange={e=>setRecurrence(e.target.value)}>{['none','daily','weekly','monthly'].map(v=><option key={v} value={v}>{repeatLabel(v)}</option>)}</select></label></div>
    ...
    {lead?<p className="ft-task-muted">Linked restaurant: {lead.business}</p>:<div className="ft-task-form-grid"><label>Find restaurant (optional)<input value={leadSearch} onChange={e=>{setLeadSearch(e.target.value);setLinkedLead('')}} disabled={admin&&!assigned} placeholder="Search assigned leads" maxLength={180}/></label><label>Linked restaurant<select disabled={(admin&&!assigned)||leadLoading} value={linkedLead} onChange={e=>setLinkedLead(e.target.value)}><option value="">{leadLoading?'Loading…':'No linked lead'}</option>{leadOptions.map(l=><option key={l.id} value={l.id}>{l.business_name}</option>)}</select></label></div>}
   ...
   <div className="ft-task-filters">
    <label>Search<input type="search" value={search} onChange={e=>setSearch(e.target.value)} placeholder="Task or restaurant" maxLength={180}/></label>
    {admin&&<label>Employee<select value={employeeFilter} onChange={e=>changeFilter(setEmployeeFilter,e.target.value)}><option value="">All employees</option>{employees.map(s=><option key={s.id} value={s.id}>{s.full_name}{s.is_active?'':' (inactive)'}</option>)}</select></label>}
    <label>Priority<select value={priorityFilter} onChange={e=>changeFilter(setPriorityFilter,e.target.value)}><option value="">All priorities</option><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></label>
    <label>Progress<select value={progressFilter} onChange={e=>changeFilter(setProgressFilter,e.target.value)} disabled={filter==='completed'}><option value="">All progress</option><option value="pending">To do</option><option value="in_progress">In progress</option>{filter==='all'&&<option value="completed">Completed</option>}</select></label>
    <label>Due from · IST<input type="date" value={from} onChange={e=>changeFilter(setFrom,e.target.value)}/></label><label>Due through · IST<input type="date" value={through} min={from} onChange={e=>changeFilter(setThrough,e.target.value)}/></label>
   </div>
   ...
   {selectedId&&<aside className="ft-task-detail" aria-label="Task details">
    ...
    {detailLoading?<p role="status">Loading details…</p>:detailError?<div role="alert"><p className="ft-task-error">{detailError}</p><button onClick={()=>setRevision(v=>v+1)}>Retry</button></div>:detail&&<>
     ...
     {detail.status==='completed'?<div className="ft-task-completed"><strong>Completed {time(detail.completed_at)} IST</strong><p>{detail.completion_note||'No completion note added.'}</p></div>:<div className="ft-task-actions">
      <button disabled={busy} onClick={()=>mutate(()=>api.taskStatus(detail.id,detail.status==='in_progress'?'pending':'in_progress'),'Task progress updated.')}>{detail.status==='in_progress'?'Move to To do':'Start task'}</button>
