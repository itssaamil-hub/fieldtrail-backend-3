...
export function TasksModal({lead,onClose,startCreate=false,focusId,embedded=false,active=true}) {
 ...
 useEffect(()=>{if(!embedded||!active)return;const refresh=()=>setRevision(v=>v+1);window.addEventListener("fieldtrail:tasks",refresh);return()=>window.removeEventListener("fieldtrail:tasks",refresh)},[embedded,active]);
 ...
 const content = <div className={embedded?"ft-tasks-embedded":"ft-tasks-backdrop"} onClick={e=>{e.stopPropagation();if(!embedded&&!busy)onClose()}}><section className="ft-tasks-dialog" role={embedded?undefined:"dialog"} aria-modal={embedded?undefined:true} aria-labelledby="ft-tasks-title" ref={dialog} onClick={e=>e.stopPropagation()}>
 <header><h2 id="ft-tasks-title">{lead?'Lead tasks':admin?'Team Tasks':'My Tasks'}</h2>{!embedded&&<button type="button" aria-label="Close tasks" onClick={onClose} disabled={busy}><X size={19}/></button>}</header>
